...
TRANSLATIONS = {
    "ja_JP": {
        ...
        (_DEFAULT, "Output Map Category"): "出力マップ分類",
        (_DEFAULT, "PBR Maps"): "PBR マップ",
        (_DEFAULT, "PBR texture maps for Principled BSDF and external tools"): "Principled BSDFや外部ツールで利用するPBRテクスチャマップ",
        (_DEFAULT, "Cycles Standard"): "Cycles 標準",
        (_DEFAULT, "Blender's standard Cycles bake passes"): "Blender標準のCyclesベイクパス",
        (_DEFAULT, "PBR Output Type"): "PBR出力形式",
        (_DEFAULT, "Individual Maps"): "個別マップ",
        (_DEFAULT, "Bake individual PBR texture maps"): "PBRテクスチャマップを個別にベイク",
        (_DEFAULT, "Packed Output"): "パック出力",
        (_DEFAULT, "Pack PBR data into RGBA presets"): "PBRデータをRGBAプリセットへパック",
        (_DEFAULT, "Selected Maps: {count}"): "選択中のマップ: {count}",
        (_DEFAULT, "No maps selected."): "マップが選択されていません。",
        (_DEFAULT, "Base Color"): "ベースカラー",
        (_DEFAULT, "Metallic"): "メタリック",
        (_DEFAULT, "Alpha"): "アルファ",
        (_DEFAULT, "Smoothness"): "スムーズネス",
        (_DEFAULT, "Emission"): "エミッション",
        (_DEFAULT, "Bakes the direct Principled Base Color input. Requires Principled BSDF connected directly to Material Output."): "Principled BSDFのベースカラー入力を直接ベイクします。Material OutputにPrincipled BSDFが直接接続されている必要があります。",
        (_DEFAULT, "Bakes the Principled Metallic input. Requires Principled BSDF connected directly to Material Output."): "Principled BSDFのメタリック入力をベイクします。Material OutputにPrincipled BSDFが直接接続されている必要があります。",
        (_DEFAULT, "Bakes the Principled Alpha input. Requires Principled BSDF connected directly to Material Output."): "Principled BSDFのアルファ入力をベイクします。Material OutputにPrincipled BSDFが直接接続されている必要があります。",
        (_DEFAULT, "Bakes inverted Principled Roughness (1 - Roughness). Requires Principled BSDF connected directly to Material Output."): "Principled BSDFの反転した粗さ（1 - Roughness）をベイクします。Material OutputにPrincipled BSDFが直接接続されている必要があります。",
        (_DEFAULT, "Material '{mat_name}' does not use nodes."): "マテリアル「{mat_name}」はノードを使用していません。",
        (_DEFAULT, "Material '{mat_name}' has no active Material Output node."): "マテリアル「{mat_name}」にアクティブなマテリアル出力ノードがありません。",
        (_DEFAULT, "Material '{mat_name}' must have a single Principled BSDF directly connected to Material Output."): "マテリアル「{mat_name}」は、マテリアル出力に単一のPrincipled BSDFが直接接続されている必要があります。",
        (_DEFAULT, "Principled BSDF in material '{mat_name}' is missing the '{socket_name}' input."): "マテリアル「{mat_name}」のPrincipled BSDFに「{socket_name}」入力がありません。",
        (_DEFAULT, "PBR map baking requires at least one material assigned to the source model(s)."): "PBRマップのベイクには、転写元モデルに少なくとも1つのマテリアルが割り当てられている必要があります。",
        (_DEFAULT, "ORM (AO, Roughness, Metallic)"): "ORM（AO、粗さ、メタリック）",
        (_DEFAULT, "glTF Metallic-Roughness"): "glTF メタリック／粗さ",
        (_DEFAULT, "Unity Mask Map"): "Unity マスクマップ",
        (_DEFAULT, "Base Color + Alpha"): "ベースカラー + アルファ",
        (_DEFAULT, "Pack Occlusion (R), Roughness (G), Metallic (B) for Godot and Unreal Engine."): "GodotやUnreal Engine向けに、AO (R)、粗さ (G)、メタリック (B) をパックします。",
        (_DEFAULT, "Pack Roughness (G) and Metallic (B) according to the glTF 2.0 standard."): "glTF 2.0規格に準拠し、粗さ (G) とメタリック (B) をパックします。",
        (_DEFAULT, "Pack Metallic (R), AO (G), Detail (B), Smoothness (A) for Unity Lit shaders."): "UnityのLitシェーダー向けに、メタリック (R)、AO (G)、詳細 (B)、スムーズネス (A) をパックします。",
        (_DEFAULT, "Pack Base Color (RGB) and Alpha (A) into a single 4-channel image."): "ベースカラー (RGB) とアルファ (A) を1枚の4チャンネル画像にパックします。",
        (_DEFAULT, "R: AO | G: Roughness | B: Metallic | A: 1.0"): "R: AO | G: 粗さ | B: メタリック | A: 1.0",
        (_DEFAULT, "R: 1.0 | G: Roughness | B: Metallic | A: 1.0"): "R: 1.0 | G: 粗さ | B: メタリック | A: 1.0",
        (_DEFAULT, "R: Metallic | G: AO | B: 0.0 | A: Smoothness"): "R: メタリック | G: AO | B: 0.0 | A: スムーズネス",
        (_DEFAULT, "RGB: Base Color | A: Alpha"): "RGB: ベースカラー | A: アルファ",
        (_DEFAULT, "Standard PBR Maps"): "基本 PBR マップ",
        (_DEFAULT, "Advanced PBR Inputs (Raw)"): "高度な PBR 入力（生値）",
        (_DEFAULT, "Show advanced raw Principled BSDF input maps"): "Principled BSDFの高度な生入力マップを表示します。",
        (_DEFAULT, "Transmission Weight"): "透過ウェイト",
        (_DEFAULT, "Specular IOR Level"): "スペキュラ IOR レベル",
        (_DEFAULT, "Coat Weight"): "コートウェイト",
        (_DEFAULT, "Coat Roughness"): "コート粗さ",
        (_DEFAULT, "Sheen Weight"): "シーンウェイト",
        (_DEFAULT, "Subsurface Weight"): "表面下散乱ウェイト",
        (_DEFAULT, "Bakes raw Principled Transmission Weight. Requires Principled BSDF connected directly to Material Output."): "Principled BSDFの透過ウェイトを生値でベイクします。Material OutputにPrincipled BSDFが直接接続されている必要があります。",
        (_DEFAULT, "Bakes raw Principled Specular IOR Level; it is not F0 itself. Requires Principled BSDF connected directly to Material Output."): "Principled BSDFのスペキュラIORレベルを生値でベイクします（F0そのものではありません）。Material OutputにPrincipled BSDFが直接接続されている必要があります。",
        (_DEFAULT, "Bakes raw Principled Coat Weight. Requires Principled BSDF connected directly to Material Output."): "Principled BSDFのコートウェイトを生値でベイクします。Material OutputにPrincipled BSDFが直接接続されている必要があります。",
        (_DEFAULT, "Bakes raw Principled Coat Roughness. Requires Principled BSDF connected directly to Material Output."): "Principled BSDFのコート粗さを生値でベイクします。Material OutputにPrincipled BSDFが直接接続されている必要があります。",
        (_DEFAULT, "Bakes raw Principled Sheen Weight. Requires Principled BSDF connected directly to Material Output."): "Principled BSDFのシーンウェイトを生値でベイクします。Material OutputにPrincipled BSDFが直接接続されている必要があります。",
        (_DEFAULT, "Bakes raw Principled Subsurface Weight. Requires Principled BSDF connected directly to Material Output."): "Principled BSDFの表面下散乱ウェイトを生値でベイクします。Material OutputにPrincipled BSDFが直接接続されている必要があります。",
        (_DEFAULT, "Required source map is unavailable: {map}"): "必須のソースマップを利用できません: {map}",
    }
}
