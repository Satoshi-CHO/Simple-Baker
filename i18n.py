...
TRANSLATIONS = {
    "ja_JP": {
        ...
        (_DEFAULT, "Bakes surface normal directions as RGB. Use through a Normal Map node when connecting it to a shader."): f"表面法線の方向をRGB画像としてベイクします。シェーダーへ接続する場合は{_JA_TERMS['normal_map_node']}を経由して使用します。",
        ...
        (_DEFAULT, "Bakes the material roughness pass for use with a shader's Roughness input."): f"シェーダーの{_JA_TERMS['roughness']}入力で使用するマテリアルの粗さをベイクします。",
        (_DEFAULT, "Bakes the material emission or glow color for use with a shader's Emission Color input."): f"シェーダーの{_JA_TERMS['emission_color']}入力で使用するマテリアルの発光色をベイクします。",
